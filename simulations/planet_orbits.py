...
limit = 1.8*3
ax = plt.axes([0., 0., 1., 1.], xlim=(-limit, limit), ylim=(-limit, limit))
ax.set_aspect('equal')
ax.axis('off')
ss = SolarSystem(Object("Sun", 28, 'red', [0, 0, 0], [0, 0, 0]))
ss.time = Time(sim_start_date).jd
colors = ['gray', 'orange', 'blue', 'chocolate', 'green']
sizes = [0.38, 0.95, 1., 0.53, 1]
names = ['Mercury', 'Venus', 'Earth', 'Mars', 'Jupiter']
texty = [.47, .73, 1, 1.5, 1]
for i, nasaid in enumerate(range(1,6)):  # The 1st, 2nd, 3rd, 4th planet in solar system
    horizons = Horizons(id=nasaid, location="@sun", epochs=ss.time, id_type='id')
    print(horizons)
    obj = horizons.vectors()
    print(obj)
